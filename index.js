...
Prompt.prototype._run = function( cb ) {
  ...
  outcome.drill.takeUntil( outcome.done ).forEach( this.handleDrill.bind(this) );
  outcome.back.takeUntil( outcome.done ).forEach( this.handleBack.bind(this) );
...
Prompt.prototype.handleSubmit = function (e) {
  ...
  var done = obx.filter((choice) => {
    //this.currentPath = path.join(this.currentPath, choice);
    var stats = fs.lstatSync(this.currentPath);
...
Prompt.prototype.createChoices = function (basePath) {
  var choices = getDirectories(basePath, this.opt.ext);
  if (choices.length > 0) {
 ...
 * @param  {String} ext file extension for filter
 * @return {Array}           array of folder names inside of basePath
 */
function getDirectories(basePath, ext) {
  return fs
    ...
    .filter(function(file) {
      ...
      var isDir = stats.isDirectory() || (stats.isFile() && path.extname(path.join(basePath, file)) === ext);
